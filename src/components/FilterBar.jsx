...
export default function FilterBar({ filters, setFilters, uniqueAssignees, currentView, setCurrentView, isOpen, onClose }) {
    ...
    const assigneeOptions = [
        ...
        ...uniqueAssignees.map(assignee => ({ value: assignee, label: `🧙‍♂️ ${assignee}` }))
    ];
    ...
    return (
        <>
            {/* Dark background overlay for mobile modal */}
            {isOpen && <div className="filter-modal-overlay" onClick={onClose}></div>}

            {/* Container dynamically adds 'modal-open' class based on state */}
            <div className={`filter-container ${isOpen ? 'modal-open' : ''}`}>
                <div className="filter-top-row">
                    ...
                    {/* NEW: Mobile close button inside the modal */}
